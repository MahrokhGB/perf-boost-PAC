...
class LQLossFHMultiBatch():
    ...
    def forward(self, xs, us):
        ...
        compute loss
        Args:
            - xs: tensor of shape (*batch_dim, T, state_dim)
            - us: tensor of shape (*batch_dim, T, in_dim)
        """
        if self.xbar is not None:
        ...
        # average over the samples which is the last batch dim
        loss_val = torch.sum(loss_val, -4)/xs.shape[-4]       # shape = (*batch_dim[0:-1], 1, 1)
        return loss_val
