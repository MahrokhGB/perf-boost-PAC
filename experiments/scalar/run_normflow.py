...
# ------------ 2. Plant ------------
sys = LTISystem(
    A = torch.tensor([[0.8]]).to(device),  # state_dim*state_dim
    B = torch.tensor([[0.1]]).to(device),  # state_dim*in_dim
    C = torch.tensor([[0.3]]).to(device),  # num_outputs*state_dim
    x_init = 2*torch.ones(1, 1).to(device),# state_dim*1
)
...
logger.info('\n[INFO] evaluating the base distribution on %i training rollouts.' % num_rollouts)
...
logger.info(msg)
# evaluate on the train data
logger.info('\n[INFO] evaluating the initial flow on %i training rollouts.' % num_rollouts)
train_loss, train_num_col = eval_norm_flow(
