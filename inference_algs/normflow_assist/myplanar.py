...
class MyPlanar(normflows.flows.base.Flow):
    ...
    def inverse(self, z):
        # original code is below. only implemented for leaky_relu activation
        ...
        # an easy fix can be made using the forward function.
        lin = torch.sum(self.w * z, list(range(1, self.w.dim()))) + self.b
        inner = torch.sum(self.w * self.u)
        u = self.u + (torch.log(1 + torch.exp(inner)) - 1 - inner) \
            * self.w / torch.sum(self.w ** 2)  # constraint w.T * u > -1
        ...
        # new
        dims = [-1] + (u.dim() - 1) * [1]
        u_ = h_(lin.reshape(-1)).reshape(*dims) * u
        inner_ = torch.sum(self.w * u_, list(range(1, self.w.dim())))
        z_ = z - u_ * (lin / (1 + inner_)).reshape(*dims)

        log_det = - torch.log(torch.abs(1 + torch.sum(self.w * u) * h_(lin.reshape(-1))))   # change from forward: added - at the beginning
        return z_, log_det


    def old_inverse(self, z):
        if self.act != "leaky_relu":
            raise NotImplementedError("This flow has no algebraic inverse.")
        lin = torch.sum(self.w * z, list(range(1, self.w.dim()))) + self.b
        a = (lin < 0) * (
